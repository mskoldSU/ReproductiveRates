...
For some organisms, such as many large mammals, there can be strong biological information about the to what extent a population can grow during a limited period of time, but this information is rarely used in analyses of data.
...
In the following, we will refer to model (1) as the _exponential_ rate model and model (2), with the logistic function parameterized as $\text{logistic}(x)=\lambda_{max}/(1 + (\lambda_{max}-1)\exp(-\lambda_{max}x/(\lambda_{max}-1)))$, as the _logistic_ rate model.
...
However, with todays computational techniques, such as MCMC via jags used here, model fitting should not be a major obstacle to implementation of the method.
