...
- \linenumbers
csl: mee.csl
bibliography: MS Bounded rmax.bib
---
...
*Abstract*: Time series models applied in the study of animal population dynamics commonly assume linearity on the log-scale, leading to log-normally distributed rates of increase. While this is often computationally convenient, in particular when performing statistical inference in the presence of observation error, it may lead to unrealistic predictions for animals with a limited reproduction. We introduce a model that includes an explicit bound on the reproductive rate of an individual, and apply this to a population time series of ungulates in Kruger National Park, South Africa. We conclude that our model can provide more realistic predictions and greater robustness against outlying environmental covariates than traditional log-linear models. We further conclude that introducing a bound on reproduction can greatly assist in separating observational error and population process noise, a key challenge in state-space modelling. 

\newpage

## Introduction

Analyses of population time series forms one of the most fundamental tools in the study of population dynamics and in population viability analyses. Time series analyses are used in many ecological studies with widely varying aims. From estimating population densities [@moore_bayesian_2011], density dependence [@dennis_estimating_2006], effects of environmental variables to extinction risks and population viability [@holmes_statistical_2007].

A common approach to modelling population time series data is to start with a discrete time deterministic population model, such as an exponential growth model, a Ricker or a Gompertz model [@brannstrom_stochastic_2006]. To account for variation, the deterministic model is converted to a stochastic model by incorporating noise often called process error [@dennis_estimation_1991], representing environmental or demographic variation [@engen_demographic_1998]. Environmental process error is, with a few exceptions, incorporated by adding a Gaussian error term to the exponential growth rate yielding a log-normal distribution for the process error at the original scale [@dennis_estimation_1991; @de_valpine_fitting_2002; @clark_population_2004].
Demographic process error is typically incorporated either by adding a Gaussian error term to the log annual growth rate, scaled by the square root of population size [@saether_climate_2006], or using a Poisson or binomial distribution for the number of births and survival events. In addition to modelling stochastic process variation, environmental variation is also modelled by introducing environmental covariates such as rain, climate or other factors thought to affect population growth rates [@stenseth_ecological_2002; @knape_effects_2011]. This is typically done in the same way as for process error by adding the covariates to the exponential growth rate, although the effects of environmental covariates on other parameters of population models have also sometimes been considered [@stenseth_ecological_2002; @mansson_time_2007].

A consequence of assuming a log-normal distribution for the process error and adding covariates at the scale of log-transformed annual growth rates is that some of the annual growth rates suggested by a fitted population model can be unrealistically large, even when the estimated value of the stochastic growth rate is realistic. 
The log-normal distribution is right skewed and heavy-tailed, implying that occasional large deviations from the median growth rate is likely to occur in the annual realized growth rates. This property of the log-normal distribution becomes more pronounced with increased variance in the process error.

Estimating the magnitude of process error from time series data is non-trivial since measurement errors typically also are present. Accounting for measurement errors is vital for many types of inference from time-series data such as estimating density dependence and population viability [@freckleton_census_2006; @holmes_statistical_2007]. State-space models incorporate both sources of errors and are often used to separate them,  but in practice it is often difficult to achieve a good separation [@dennis_estimating_2006; @knape_estimability_2008].

Here, we suggest a simple modification of the Gompertz state-space model to improve the separation of measurement and process error in state-space models for population time series by making use of biological information about maximum growth rates. For some types of organisms, large mammals in particular, there is often strong biological information about the how much a population can grow during a limited period of time, but it is rarely used in analyses of data. Some previous studies have explored incorporating prior knowledge in the stochastic growth rate parameter of population models, i.e. the expected mean growth rate on the log-scale when the population is small [@delean_ecologically_2012; @lebreton_detecting_2013].
Our approach is instead based on bounding the realized rather than the stochastic growth rate using biological knowledge.
It's effects are demonstrated in applications to data on ungulate populations in the Kruger national park.
...
The log-linear form of the population process in (1) makes it convenient to include environmental covariates like $z_t$, but also implies the unbounded increase or decline of log-abundance. Large mammals often have a sharp upper limit in their reproductive rates; if fertile females in a closed population produce at most one juvenile per season, $\lambda_t=N_{t+1}/N_{t}$ is clearly bounded from above by $\lambda_{max}=1+p$, where $p$ is the population proportion of fertile females. Higher rates could be possible if we allow immigration, in this study we assume effects of migration to be negligible though. 
...
The logistic function, $\text{logistic}(x)=L/(\exp(-b(x-a))+1)$, is the sigmoid function decreasing to zero as $x$ approaches $-\infty$ and increasing to  $L$ as $x$ approaches $\infty$. Here, the parameter $b>0$ controls the steepness of the curve and $a$ the location of its midpoint. Its use in this paper resembles the way it is used in statistics, in particular _logistic regression_, as a mean of transforming the effect of a linear combination of covariates to a bounded interval. This should not be confused with its rather different and traditional appearance in population ecology through Verhulst's _logistic model of population growth_. In Verhulst's model, it forms a bound on a population's _size_ through carrying capacity $L=K$. Our aim is instead to implement a bound $L=\lambda_{max}$ on the population's _rate of increase_. In agreement with (1) we do so by simply replacing the exponential with the logistic, 
$$
N_t=N_{t-1}\text{logistic}(\beta v_t+\epsilon_t),\quad t=1,\ldots,T,\quad (2)
$$
where $\epsilon_t$ is again Gaussian noise with zero mean and variance $\sigma^2$. In order for the parameters $\beta$ and $\sigma^2$  to be comparable with those of the exponential model, we set $L=\lambda_{max}$ and fix location and scale of the logistic function at $a=\log(\lambda_{max}-1)/b$ and $b=\lambda_{max}/(\lambda_{max}-1)$. This choice ensures that the exponential and logistic functions have the same value and first derivative at $x=0$, see Figure 1 for the case $\lambda_{max}=1.5$. It also ensures that the carrying capacities of the two models are the same.
...
As $\lambda_{max}$ increases, the logistic function parametrised as above will approach the exponential function so that the Gompertz model is recovered in the limit of model (2) when the remaining parameters are held fixed.

In the following, we will refer to model (1) as the _exponential_ rate model and model (2), with the logistic function parametrised as $\text{logistic}(x)=\lambda_{max}/(1 + (\lambda_{max}-1)\exp(-\lambda_{max}x/(\lambda_{max}-1)))$, as the _logistic_ rate model.
...
We have analyzed 7 of the species covered by the census; waterbuck (*Kobus ellipsiprymnus*), giraffe (*Giraffa camelopardalis*), Burchell’s zebra (*Equus burchelli*), greater kudu (*Tragelaphus strepsiceros*), blue wildebeest (*Connochaetes taurinus*), impala (*Aepyceros melampus*) and sable antelope (*Hippotragus niger*). In the paper we report results for the giraffe and the waterbuck as examples of species with high and low detection probabilities respectively. @redfern_biases_2002 reports a detection probability of ~50% for the waterbuck. Results for the other species are found in the supplementary material. 
...
Regional census totals are shown in Figure 2 together with dry season rainfall. The waterbuck in particular show a decreasing trend in response to the lower precipitation from 1988 and onwards. 
...
\end{cases}
$$
where $v_{it}=(1,\log(N_{i(t-1)}),z_{it})^T$ with $z_{it}$ denoting dry season rainfall in region $i$ year $t$, $\epsilon_{it}\sim N(0,\sigma^2)$ independently over indices and finally $\text{logistic}(x)=\lambda_{max}/(\exp(-\lambda_{max}x/(\lambda_{max}-1))/(\lambda_{max}-1)+1)$ given a known upper bound $\lambda_{max}$ on the reproductive rate of the species. Parameters are given vague prior distributions, see Supplementary material. As a further simplification, we start each population process at it's first observed value, $N_{it_{i0}}=y_{it_{i0}}$.

Since we are mainly interested in comparing the exponential and logistic rate models, we have kept the remaining model details as simple as possible while still including a continuous covariate represented by dry season rainfall. @mansson_time_2007 consider the same model structure as our exponential rate model, but with a wider range of covariates and higher order time lags of density dependence. Also @ogutu_enso_2003 try a wider range of covariates, but they only model park totals and their model do not include a measurement error.

### Bounds on reproductive rates

A true upper bound on reproduction depend, among other factors, on proportion of reproductive females, litter sizes, gestation periods and juvenile survival. With the exeption of the giraffe, which has a gestation period of 15 months, reproduction for the species under study is mainly  seasonal with only rare occasions of twins. We base our maximal reproductive rates on summary population structure statistics in @owen-smith_comparative_2005. They report average number of juveniles ($p_j$), yearlings ($p_y$) and adult males ($p_m$) per adult female across the full census. This is used to estimate proportion of adult females as $p=1/(1+p_j+p_y+p_m)$, and set $\lambda_{max}=1+p$. For the giraffe we assume the number of subadults to equal three times the number of two-year olds, since data is only available on the latter. Further, the maximal rate is corrected for the giraffe's long gestation period as $\lambda_{max}=1+p\times 12/15$. Since for all species under study estimated upper bounds range between 1.4 and 1.5, we finally decide to use a common value of $\lambda_{max}=1.5$.


## Results

Posterior densities for all species and parameters are available in the supplementary material, we here focus on our key findings.

### Separating process noise and observation error

A key challenge in state-space modelling lies in separating process noise and observation error. As noted by e.g. Dennis et.al. (2006), the likelihood function often have local maxima along $\sigma = 0$ and $s = 0$, and profile likelihoods for $(\sigma, s)$ tend to be ridge-shaped due to the difficulties in separating the two contributions to variation. For the giraffe, this poses no problem and both approaches give similar posterior densities for $(\sigma, s)$, clearly bounded away from zero (see supplemental material). For the waterbuck, on the other hand, the log-normal model shows a ridge-shaped density and fails to confirm that observation error exist (Figure 3). This is also observed for the Kudu, Blue Wildebeest and to lesser extend the Zebra (see Suppelemntary material).
...
In 1984, 301 waterbuck were counted in the Far North region. The next year almost twice as many, 599, were counted. This is far above the maximal reproduction, and given that also the other districts increase over the period much of the increase could be attributed to sampling error rather than migration. As noted by @redfern_biases_2002, detection probabilities can differ greatly between years due to variations in vegetation cover.   Figure 4a shows the posterior distribution for the total population index under the two models, here the logisitic rate model predict a smoother curve while the exponential rate model seems to overfit data resulting in an unrealistic growth rate in 1984-1985. For the giraffe, the results from the two models are difficult to distinguish (Figure 4b). In Figure 5 we have plotted the corresponding posterior densities for the rates of increase of the waterbuck population from 1984 to 1985 and 1985 to 1986, here the former shows a substantial posterior probability mass on values above 1.5 for the exponential rate model.
...
![_(a) Amount of dry season rainfall (mm) in the four districts. (b) Animal counts for giraffe and waterbuck in the four districts._](Figs/counts_and_rain_fig.pdf)
...
![_Posterior median Central district population index for the waterbuck __(a)__ and giraffe __(b)__ together with 50 and 90 percent credible intervals for exponential rate model (left frame) and logistic rate model (right frame). Actual counts superimposed in black._](Figs/waterbuck_giraffe_central_fig.pdf)


![_Posterior distributions of the rate of increase of the total waterbuck population from 1984-1985  ($\lambda_{1985}$) and from 1985-1986 ($\lambda_{1986}$)._](Figs/waterbuck_rates_fig.pdf)


![_Posterior median and interquartile range of the rate of increase of waterbuck, starting at an average level and as a function of dry-season rainfall for exponential and logistic rate models. Superimposed at the bottom is a histogram of observed dry season rainfall, all years and districts._](Figs/rate_vs_rain_fig.pdf)

![_Posterior predictive waterbuck median abundance together with 50 and 90% credible intervals. In __(a)__ corresponding to the high precipitation scenario observed in 1981-1987 and in __(b)__ corresponding to the low precipitation scenario observed in 1988-1984._](Figs/high_low_prec_fig.pdf)
