...
$$
N_t=N_{t-1}\text{logistic}(\beta v_t+\epsilon_t)=N_{t-1}\lambda_t,\quad t=1,\ldots,T,\quad (2)
$$
where $\epsilon_t$ is again Gaussian noise with zero mean and variance $\sigma^2$. Since our main aim in this paper is to compare the models defined in (1) and (2), we use a particular parametrisation of the logistic function in (2). Rather than the standard choice of $a=0$ and $b=1$, leading to $\text{logit}(\lambda_t/\lambda_{max})=\beta v_t+\epsilon_t$, we set $a=\log(\lambda_{max}-1)/b$ and $b=\lambda_{max}/(\lambda_{max}-1)$. This parametrisation ensures that the exponential and logistic functions have the same value and first derivative at $x=0$, making parameters $\beta$ and $\sigma^2$ comparable across models. It also ensures that the carrying capacities of the two models are the same. When refering to the logistic function in this paper, we hence refer to the version defined by
