...
$$
where $\epsilon_t$ is again Gaussian noise with zero mean and variance $\sigma^2$. In order for the parameters $\beta$ and $\sigma^2$  to be comparable with those of the exponential model, we parametrise the logistic function as
$$
...
\end{cases}
$$
where $v_{it}=(1,\log(N_{i(t-1)}),z_{it})^T$ with $z_{it}$ denoting dry season rainfall in region $i$ year $t$, $\epsilon_{it}\sim N(0,\sigma^2)$ independently over indices and finally the logistic function parametrised as in (3) given a known upper bound $\lambda_{max}$ on the reproductive rate of the species. All parameters are given vague prior distributions, see Supplementary material. As a further simplification, we start each population process at it's first observed value, $N_{it_{i0}}=y_{it_{i0}}$.
